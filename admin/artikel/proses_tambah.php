...
class ArtikelUpload extends Artikel {
    public function tambahArtikel() {
        ...
        $judul = $this->escapeString(isset($_POST['judul']) ? $_POST['judul'] : '');
        $id_kategori = $this->escapeString(isset($_POST['id_kategori']) ? $_POST['id_kategori'] : '');
        $isi = $this->escapeString(isset($_POST['isi']) ? $_POST['isi'] : '');
        $tanggal = date('Ymd');
        ...
        if($_FILES['foto']['error'] == 0){
            if($_FILES['foto']['size'] < 1000000){
                ...
                $query = mysqli_query($this->koneksi, "INSERT INTO tbl_artikel (judul, isi, tanggal, foto, id_kategori) VALUES('$judul', '$isi', '$tanggal', '$nama_foto', $id_kategori)") or die(mysqli_error($this->koneksi));
...
$artikelUpload = new ArtikelUpload($koneksi);
$artikelUpload->tambahArtikel();
?>
